...
fun parseTraktContentIds(contentId: String?): TraktExternalIds {
...
fun normalizeTraktContentId(ids: TraktExternalIds?, fallback: String? = null): String {
...
fun extractTraktYear(value: String?): Int? {
    if (value.isNullOrBlank()) return null
    return Regex("(\\d{4})").find(value)?.groupValues?.getOrNull(1)?.toIntOrNull()
}

fun TraktExternalIds.hasAnyId(): Boolean =
 ...
 * remote responses.
 */
fun isTraktCompatibleId(contentId: String?): Boolean {
 ...
 */
fun resolveEffectiveContentId(contentId: String, videoId: String?): String {
