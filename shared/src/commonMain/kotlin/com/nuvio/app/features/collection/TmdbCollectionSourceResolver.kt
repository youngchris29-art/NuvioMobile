...
object TmdbCollectionSourceResolver {
        ...
        withContext(Dispatchers.Default) {
            ...
            when (sourceType) {
                TmdbCollectionSourceType.LIST -> {
                    ...
                    ) ?: error(resourceString("TMDB list not found", StringKey.collections_tmdb_list_not_found))
                ...
                TmdbCollectionSourceType.COMPANY -> {
                    val body = fetch<TmdbCompanyResponse>(
                        endpoint = "company/$id",
                        apiKey = apiKey,
                    ) ?: error(resourceString("TMDB company not found", StringKey.collections_tmdb_company_not_found))
                    TmdbSourceImportMetadata(
                        title = body.name?.takeIf { it.isNotBlank() },
                ...
                TmdbCollectionSourceType.NETWORK -> {
                    val body = fetch<TmdbNetworkResponse>(
                        endpoint = "network/$id",
                        apiKey = apiKey,
                    ) ?: error(resourceString("TMDB network not found", StringKey.collections_tmdb_network_not_found))
                    TmdbSourceImportMetadata(
                        title = body.name?.takeIf { it.isNotBlank() },
                        coverImageUrl = imageUrl(body.logoPath, "w500"),
                    )
                ...
                TmdbCollectionSourceType.DIRECTOR -> {
                    val body = fetch<TmdbPersonResponse>(
                        ...
                        apiKey = apiKey,
                        query = mapOf("language" to language),
                    ) ?: error(resourceString("TMDB person not found", StringKey.collections_tmdb_person_not_found))
                    TmdbSourceImportMetadata(
                        title = body.name?.takeIf { it.isNotBlank() },
                ...
                TmdbCollectionSourceType.DISCOVER -> TmdbSourceImportMetadata(title = resourceString("TMDB Discover", StringKey.collections_editor_tmdb_discover))
            }
        }
    ...
    private suspend fun resolveList(
        ...
        val id = source.tmdbId ?: error(resourceString("Missing TMDB list ID", StringKey.collections_tmdb_missing_list_id))
        val body = fetch<TmdbListResponse>(
            endpoint = "list/$id",
            apiKey = apiKey,
    ...
    private suspend fun resolveCollection(
        ...
        val id = source.tmdbId ?: error(resourceString("Missing TMDB collection ID", StringKey.collections_tmdb_missing_collection_id))
        val body = fetch<TmdbCollectionResponse>(
            endpoint = "collection/$id",
            apiKey = apiKey,
            query = mapOf("language" to language),
    ...
    private suspend fun resolvePersonCredits(
        ...
        val id = source.tmdbId ?: error(resourceString("Missing TMDB person ID", StringKey.collections_tmdb_missing_person_id))
        val mediaType = source.tmdbMediaType()
        val body = fetch<TmdbPersonCreditsResponse>(
            endpoint = "person/$id/combined_credits",
            apiKey = apiKey,
            query = mapOf("language" to language),
        ) ?: error(resourceString("TMDB person credits not found", StringKey.collections_tmdb_person_credits_not_found))
    ...
    private suspend fun resolveDiscover(
        ...
        val body = fetch<TmdbDiscoverResponse>(
            ...
            apiKey = apiKey,
            query = query,
        ) ?: error(resourceString("TMDB discover returned no data", StringKey.collections_tmdb_discover_no_data))
