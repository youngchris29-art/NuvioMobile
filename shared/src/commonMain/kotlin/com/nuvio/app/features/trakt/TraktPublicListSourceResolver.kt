...
object TraktPublicListSourceResolver {
    ...
    suspend fun resolve(source: CollectionSource, page: Int = 1): CatalogPage = withContext(Dispatchers.Default) {
        val listId = source.traktListId?.takeIf { it > 0L } ?: error(resourceString("Missing Trakt list ID", StringKey.collections_trakt_missing_list_id))
        ...
        if (response.status !in 200..299) {
            error(errorMessageFor(response.status, resourceString("Could not load Trakt list", StringKey.collections_editor_trakt_load_failed)))
        }
    ...
    suspend fun listImportMetadata(input: String): TraktPublicListImportMetadata = withContext(Dispatchers.Default) {
        val idPath = parseTraktListPath(input) ?: error(resourceString("Enter a valid Trakt list ID or URL", StringKey.collections_trakt_invalid_list_id_or_url))
        val list = requestJson<PublicTraktListSummaryDto>(
            endpoint = "lists/$idPath",
            query = mapOf("extended" to "full,images"),
        )
        val id = list.ids?.trakt ?: idPath.toLongOrNull() ?: error(resourceString("Trakt list did not include a numeric ID", StringKey.collections_trakt_missing_numeric_id))
    ...
    private suspend inline fun <reified T> requestJson(
        ...
        if (response.status !in 200..299) {
            error(errorMessageFor(response.status, resourceString("Trakt request failed", StringKey.collections_trakt_request_failed)))
        }
    ...
    private suspend fun requestRaw(
        ...
        if (TraktConfig.CLIENT_ID.isBlank()) {
            error(resourceString("Missing Trakt credentials.", StringKey.collections_trakt_credentials_missing))
        }
    ...
    private fun PublicTraktListSummaryDto.toPublicListResult(likeCount: Int? = null): TraktPublicListSearchResult? {
        ...
        return runBlocking {
            val listTitle = name?.takeIf { it.isNotBlank() }
                ?: resourceString("Trakt List ${id}", StringKey.collections_editor_trakt_fallback_title, id)
            val owner = user?.username?.takeIf { it.isNotBlank() }
            val stats = buildList {
                itemCount?.let { add(resourceString("${it} items", StringKey.collections_trakt_list_items_count, it)) }
                (likeCount ?: likes)?.let { add(resourceString("${it} likes", StringKey.collections_trakt_list_likes_count, it)) }
            }
            val subtitle = (listOfNotNull(owner) + stats).joinToString(" • ")
                .ifBlank { resourceString("Trakt public list", StringKey.collections_trakt_public_list) }
    ...
    private fun errorMessageFor(code: Int, fallback: String): String = runBlocking {
        when (code) {
            401, 403, 404 -> resourceString("Trakt list not found or not public", StringKey.collections_trakt_list_not_found_or_private)
            429 -> resourceString("Trakt rate limit reached", StringKey.collections_trakt_rate_limit_reached)
            else -> resourceString("${fallback} (${code})", StringKey.collections_trakt_error_with_code, fallback, code)
        }
    }
}
