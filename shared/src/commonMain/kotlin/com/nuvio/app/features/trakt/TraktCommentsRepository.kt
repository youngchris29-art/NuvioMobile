    ...
    suspend fun getCommentsPage(
        ...
        if (response.status !in 200..299) {
            throw IllegalStateException(
                resourceString("Failed to load Trakt comments (${response.status})", StringKey.details_comments_trakt_load_failed_with_code, response.status),
            )
        }
...
private fun toReviewModel(dto: TraktCommentDto): TraktCommentReview {
    val authorDisplayName = dto.user?.name
        ...
        ?: resourceString("Trakt user", StringKey.trakt_user_fallback)
